// This code was used to determine four states of a large 
// slider window/door at P1:
// (i.e tilted, fully opened, closed or anything inbetween)
...
//Window stat(s)
const int switchDOOR = 2; //toptilted
bool DOORopen = false;

const int switchWIND1 = 4; //bottomcheck
bool WINDopen1 = false;

const int switchWIND2 = 5; //fullopen
...
//--------------------------------------------------------------------------
  ...
  iot.addFloat("HumidityWtest", humidity);
  iot.addFloat("TemperatureWtest", temperature);
  iot.addFloat("HICWtest", heat_index_celsius);
  
  if(DOORopen == true && WINDopen1 == false && WINDopen2 == true){
    iot.addInt("BigWindowTilt", 0);
    Serial.println("WindowOpenTilt");
  } else {
    iot.addInt("BigWindowTilt", 1);
    Serial.println("Nothing");
  }
  if(DOORopen == true && WINDopen1 == true && WINDopen2 == false){
    iot.addInt("BigWindowFull", 0);
    Serial.println("WindowOpenFull");
  } else {
    iot.addInt("BigWindowFull", 1);
    Serial.println("Nothing");
  }
  if(DOORopen == true && WINDopen1 == true && WINDopen2 == true){
    iot.addInt("BigWindowOpenElse", 0);
    Serial.println("Window1OpenElse");
  } else {
    iot.addInt("BigWindowOpenElse", 1);
    Serial.println("Nothing");
  }
